...
export function StoreAssetList() {
  ...
  const multiSearchApply = async (
    ...
    const res = await axios.post(`${ngrokRes.data.ngrokAddress}/assest_search`, {
      // const res = await axios.post(`http://192.168.0.28:3000/assest_search`, {
      main: bigCategory,
      page: page,
    ...
    setMultiAssetData(prstArray);
  };

  const seelectItem = (presentation: Asset) => {
  ...
  const openPreview = (item: Asset) => {
    modalCtrl.open(<AssetPreview asset={item} />);
  };
  ...
  const makeItOwn = () => {
    ...
    navigate(`/layoutEditor/new/${selectedItems[0].id}`);
  };

  const openAssetDialog = () => {
    if (selectedItems.length === 0) {
      modalCtrl.open(<Alert text='상세히 볼 자료를 선택해주세요.' />);
      return;
    }
    if (selectedItems.length > 1) {
      modalCtrl.open(<Alert text='상세히 볼 자료는 하나만 선택해주세요.' />);
      return;
    }

    modalCtrl.open(<AssetDialog asset={selectedItems[0]} />);
  };
  ...
  return (
    <Page.Container>
      ...
      <Page.Actions>
        ...
        <Page.ActionButton onClick={openAssetDialog}>
          <FontAwesomeIcon icon={faInfoCircle} />
          &nbsp; 상세정보
        </Page.ActionButton>
      ...
      <Page.Actions>
        ...
        <Menu {...searchTargetMenu.popperProps}>
          <Page.MenuItem onClick={() => navigate('/store/presentation')}>
            &nbsp; 프레젠테이션
          </Page.MenuItem>
          <Page.MenuItem>
            <FontAwesomeIcon icon={faCheck} />
        ...
        <Page.ThumbnailGrid>
          {multiAssetData.map((item) => (
            ...
            >
              ...
              <Page.ThumbnailImage
                src={config.EXTERNAL.CUBLICK.ASSET.THUMBNAIL(item.id, authToken)}
                alt={item.name}
              />
            </Page.Thumbnail>
          ))}
